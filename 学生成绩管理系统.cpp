...
using namespace std;//此程序较简单且作业要求中指明编写控制台应用程序，故而使用标准命名空间std
...
public://公有成员
	...
	void Window(int pos_x, int pos_y, int cols, int lines)//设置窗口和缓冲区大小
	{
		...
		SetWindowLongPtrA(//使控制台窗口无法更改大小，避免bug
			GetConsoleWindow(),
			GWL_STYLE,
			GetWindowLongPtrA(GetConsoleWindow(), GWL_STYLE)
			& ~WS_SIZEBOX
		);//& ~WS_MAXIMIZEBOX & ~WS_MINIMIZEBOX隐藏最大化最小化按钮
	}
	/*系统架构::设置控制台窗口与缓冲区的位置与大小结束*/

	/*系统架构::清屏*/
	/*void Cls() {//清屏函数，不太好用，仍然会导致闪屏状况发生，且目前有bug
		CONSOLE_SCREEN_BUFFER_INFO cursor;
		GetConsoleScreenBufferInfo(hStdout, &cursor);//得到当前光标位置

		SetConsoleCursorPosition(hStdout, { 0,0 });//设置光标位置为{ 0,0 }
		for (SHORT i = 0; i <= cursor.dwCursorPosition.Y ; i++)//每行都清空
		{
			for (size_t i = 1; i < 80 + 1; i++)//i绝不能=0
			{
				cout << ' ';//覆盖Backspace太长了的bug
			}
		}
		SetConsoleCursorPosition(hStdout, { 0,0 });//设置光标位置为{ 0,0 }
	}*/
	...
	void Pause(bool style)//暂停控制台输出
	{
		...
		char temp = _getch();//使用_getch()函数静候任意字符的输入，结果并无实际意义（请按任意键继续）
	}
	/*系统架构::暂停控制台输出结束*/
	...
	void Input_char(bool style)//输入字符
	{
		if (style == true)//主界面样式
		{
			...
			{
				cout << "\r";//'\r'的应用，重回本行开头
				for (size_t i = 1; i < 80; i++)//i绝不可=0，因为会导致换行
				{
					cout << ' ';//覆盖Backspace太长了的bug
				}
				cout << "\r\t\t\t\t";//'\r'的应用，重回本行开头
				...
				switch (input_char)
				{
				case '\015':cout << "当前选择[ 回车 ]键"; break;//判断是否等于回车键
				case '\033':cout << "当前选择[ Escape ]键"; break;//判断是否等于ESC键
				case '\010':cout << "当前选择[ Backspace ]键"; break;//判断是否等于退格键
				default:
					cout << "当前选择[ " << input_char << " ]键";//其他按键直接输出
					break;
				}
				/*if (input_char == '\015')
				{
	...
	void Error(bool style)//提示输入错误
	{
		if (style == true)//主界面样式
		{
			...
			switch (input_char)
			{
			case '\015':cout << "【输入错误】值为[ 回车 ]键，请重新选择 >>>>>>>> " << endl; break;//判断是否等于回车键
			case '\033':cout << "【输入错误】值为[ Escape ]键，请重新选择 >>>>>>>> " << endl; break;//判断是否等于ESC键
			case '\010':cout << "【输入错误】值为[ Backspace ]键，请重新选择 >>>>>>>> " << endl; break;//判断是否等于退格键
			default://其他按键直接输出
...
public://公有成员
	...
	double* score = new double[M];//单科成绩，类中动态申请内存，易造成crash（崩溃），需要重载赋值运算符（本想用STL，种种原因未能施行，只好手动）
...
public://公有成员
	...
	void Input()//成绩录入
	{
		...
		Window(0, 0, 80, 34);//设置窗口大小为80行34列（废弃原来64行的设定，完全没必要）
		...
		Cls();//清屏
		for (count = 0; count < M; count++)//此处用STL的话会好办很多，就不必先确定人数了，可惜我懒得改
		{
			cout << "\n·请输入第[ " << count + 1 << " ]门课的名称：";
	...
	void Output()//成绩显示，利用缓冲区9000行的优势，采用竖排全部输出
	{
		...
		Window(0, 0, 80, 34);//设置窗口大小为80列34行
		Black_White();//更新前景色、背景色
		Cls();//清屏
		Display_cursor();//显示光标
	...
	/*模块3::成绩排序*/
	void Sort()//成绩排序，因作业要求，手动实现简单排序算法，就不调库了
	{
		Title(TEXT("学生成绩管理系统::成绩排序"));//设置标题
		Window(0, 0, 80, 34);//设置窗口大小为80行34列
		Black_White();//更新前景色、背景色
		Cls();//清屏
		Display_cursor();//显示光标
		...
		if (input_char == 'u')//升序，其实这么多if...else是不妥善的方法，但我懒得改了
		{
			do//do-while循环，先执行一次
			{
				Black_White();//黑色背景，白色字体
	...
	void Save()//保存成绩
	{
		...
		Title(TEXT("学生成绩管理系统::成绩保存"));//调用Title函数设置子界面的标题
		Window(0, 0, 80, 34);//设置窗口大小
		Purple();//白色背景，紫色字体
		Cls();//清屏
		cout << "\n·正在保存..." << endl;//改为紫色字体
		...
		else//否则，一切顺利
		{
			...
			outfile << "\n==================================\n" << endl;//作为分隔符
			MessageBox(GetConsoleWindow(), TEXT("保存数据成功！文件名为本目录下的“学生成绩信息.txt”"), TEXT("保存成功"), MB_OK | MB_ICONINFORMATION | MB_DEFBUTTON1);//弹出成功信息
	...
	/*系统架构::关于信息*/
	void About(int argc, char** argv)//输出关于信息
	{
		//system("mode con cols=80 lines=32");//改变宽高，最开始的用法，因调用Windows API，故现已废弃
		//system("color f0");//改变颜色，最开始的用法，因调用Windows API，故现已废弃
		//system("title 学生成绩管理系统::关于");//更新标题，最开始的用法，因调用Windows API，故现已废弃
		Title(TEXT("学生成绩管理系统::关于"));//调用Title函数设置子界面的标题
		Window(0, 0, 80, 34);//设置窗口大小
		do//do-while循环，先执行一次
		{
			Purple();//白色背景，紫色字体
...
int main(int argc, char** argv, char** envp)//主函数，参数依次为：使用CMD窗口运行进程时输入参数个数，程序目录，系统环境变量
{
	...
	do//do-while循环，先执行一次
	{
		...
		case 'f':Function.Save(); break;//成绩保存
		case 'q':Function.About(argc, argv); break;//调用关于函数（含退出功能）输出关于信息，其中，envp忒长了，影响美观，就不传入了
		default: Build.error = true;//否则，输入错误，方便catch error
		}
	} while (1);//死循环，废除以前的Build.input_char != 'q'
